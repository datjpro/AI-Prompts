...
// --- FadingVideo Component (JS Crossfade, No CSS transitions) ---
...
function BlurText({ text, className = "", delay = 0 }) {
  ...
  return (
    ...
    >
      {words.map((word, i) => {
        const wordDelay = delay + (i * 100) / 1000;
        return (
          <motion.span
            key={i}
...
function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen w-full bg-black overflow-hidden flex flex-col justify-between">
      {/* Background Video */}
      <FadingVideo
      ...
      <nav className="fixed top-4 left-0 right-0 px-8 lg:px-16 z-50 flex items-center justify-between pointer-events-auto">
        {/* Logo */}
        <div className="w-12 h-12 liquid-glass rounded-full flex items-center justify-center font-heading italic text-2xl text-white select-none">
          a
        </div>

        {/* Desktop Nav Links */}
        <div className="hidden lg:flex items-center gap-1 liquid-glass rounded-full px-1.5 py-1.5">
          {navLinks.map((link) => (
            <a
              key={link}
          ...
          <button className="bg-white text-black px-4 py-2 text-sm font-medium rounded-full flex items-center gap-1.5 whitespace-nowrap hover:bg-white/90 transition-colors cursor-pointer">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center pt-24 px-4 text-center max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          className="liquid-glass rounded-full inline-flex items-center p-1 text-xs font-body mb-6"
        >
          <span className="bg-white text-black px-3 py-1 text-xs font-semibold rounded-full mr-2">
            New
          </span>
        ...
        >
          <button className="liquid-glass-strong rounded-full px-5 py-2.5 text-sm font-medium text-white flex items-center gap-2 font-body hover:opacity-90 transition-opacity cursor-pointer">
            <span>Start Your Voyage</span>
            <ArrowUpRightIcon className="h-5 w-5" />
          </button>
          <button className="flex items-center gap-2 text-sm font-medium text-white font-body hover:text-white/80 transition-colors cursor-pointer">
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row items-stretch justify-center gap-4 mt-8"
        >
          {/* Card 1 */}
          <div className="liquid-glass p-5 w-[220px] rounded-[1.25rem] flex flex-col justify-between text-left">
            <ClockIcon />
            <div className="mt-4">
              <div className="text-4xl tracking-[-1px] leading-none font-heading italic text-white">
                34.5 Min
              </div>
      ...
      >
        ...
        <div className="flex items-center justify-center gap-12 md:gap-16 flex-wrap text-2xl md:text-3xl tracking-tight font-heading italic text-white">
          {partners.map((partner, idx) => (
            <React.Fragment key={partner}>
              <span>{partner}</span>
              {idx < partners.length - 1 && <span className="opacity-60">·</span>}
            </React.Fragment>
          ))}
        </div>
      </motion.div>
    ...
    <section className="relative min-h-screen w-full bg-black flex flex-col justify-between overflow-hidden">
      ...
      <div className="relative z-10 px-8 md:px-16 lg:px-20 pt-24 pb-10 flex flex-col min-h-screen justify-between">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16">
          {cards.map((card, i) => (
            <div key={i} className="liquid-glass rounded-[1.25rem] p-6 min-h-[360px] flex flex-col justify-between">
              {/* Top row */}
              <div className="flex items-start justify-between gap-4">
                {/* Icon */}
                <div className="w-[44px] h-[44px] liquid-glass rounded-[0.75rem] flex items-center justify-center shrink-0">
